    ...
    <Layout>
      ...
      <Content style="display: flex; height: calc(100vh - 64px)">
        ...
        <div v-if="state.show" :class="`left-bar ${state.toolsBarShow && 'show-tools-bar'}`">
          <Menu :active-name="state.menuActive" accordion @on-select="showToolsBar" width="165px">
            <div style="padding: 20px; display: flex; flex-direction: column; align-items: center">
              <Button style="margin: auto" @click="addListView" type="primary">新增页面</Button>
              ...
              >
                <Button
                  v-for="(view, key) in listView"
                  :type="view.isActive ? 'primary' : 'none'"
                  :key="key"
                  @click="handleSelectView(key)"
...
onMounted(() => {
  ...
  event.init(canvas);
  let activeKey = null;
  const list = JSON.parse(localStorage.getItem('listView'));
  for (const key in list) {
    if (list[key].isActive) {
      activeKey = key;
    }
  }
  if (activeKey) {
    canvas.loadFromJSON(list[Number(activeKey)].history || null, function () {
      // 在加载完成后执行的代码
      canvas.renderAll();
    });
  }
...
const listArr = JSON.parse(localStorage.getItem('listView'));
const listView = ref(listArr || []);
const addListView = () => {
  listView.value = [...listView.value, { isActive: false }];
  localStorage.setItem('listView', JSON.stringify(listView.value));
};
const handleSelectView = (key) => {
  const list = JSON.parse(localStorage.getItem('listView'));
  for (let item in list) {
    list[Number(item)].isActive = Number(item) === key;
  }
  listView.value = list;
  localStorage.setItem('listView', JSON.stringify(listView.value));
  ...
  if (list[Number(activeKey)].history) {
    canvas.loadFromJSON(list[Number(activeKey)].history, function () {
      // 在加载完成后执行的代码
      canvas.renderAll();
    });
  } else {
    ...
    canvas.renderAll();
    // canvas.discardActiveObject();
    // canvas.loadFromJSON(null, function() {
    //   // 在加载完成后执行的代码
    //   canvas.renderAll();
    // });
  }
...
const handleDelete = (key) => {
  const list = JSON.parse(localStorage.getItem('listView'));
  list.splice(key, 1);
  localStorage.setItem('listView', JSON.stringify(list));
};
</script>
